...
const bfFirst = `${BF}/fr-fr/blog/la-premiere-fois-dans-une-salle-de-sport`
...
export function exerciseGuide(name=''){
  ...
  const fallback=exerciseMedia(name)
  return fallback?{...guide,src:fallback.image,url:fallback.page,source:fallback.source}:guide
}

function media(guide){
  if(guide.src)return `<a class="bf-media" href="${esc(guide.url)}" target="_blank" rel="noopener"><img src="${esc(guide.src)}" alt="Démonstration ${esc(guide.label)} — ${esc(guide.source)}" loading="lazy"><span>Démo ${esc(guide.source)}</span></a>`
  return `<a class="bf-reference" href="${esc(guide.url)}" target="_blank" rel="noopener"><div><span class="bf-source">GUIDE ${esc(guide.source)}</span><strong>${esc(guide.label)}</strong><small>Toucher pour ouvrir la démonstration complète</small></div><b>↗</b></a>`
}
