...
export function exerciseGuide(name = '') {
  ...
  return guides.find((guide) => guide.match.some((key) => normalized.includes(key))) || null
}

function media(guide) {
  if (guide.src) {
    return `<a class="bf-media" href="${esc(guide.url)}" target="_blank" rel="noopener">
      <img src="${esc(guide.src)}" alt="Démonstration ${esc(guide.label)} — ${esc(guide.source)}" loading="lazy">
      <span>Démo officielle ${esc(guide.source)}</span>
    </a>`
  }
  return `<a class="bf-reference" href="${esc(guide.url)}" target="_blank" rel="noopener">
    <div><span class="bf-source">FICHE OFFICIELLE ${esc(guide.source)}</span><strong>${esc(guide.label)}</strong><small>Toucher pour ouvrir la démonstration complète</small></div><b>↗</b>
  </a>`
}
...
export function renderExerciseGuide(name = '') {
  ...
  return `<section class="bf-machine">
    ...
    ${media(guide)}
    <div class="bf-guide">
      <div class="bf-title"><strong>${esc(guide.label)}</strong><small>${esc(guide.muscles)}</small></div>
      <div class="bf-steps">
        <div><b>1</b><p><strong>Réglage</strong><span>${esc(guide.setup)}</span></p></div>
        <div><b>2</b><p><strong>Mouvement</strong><span>${esc(guide.move)}</span></p></div>
        <div class="bf-warning"><b>!</b><p><strong>À éviter</strong><span>${esc(guide.avoid)}</span></p></div>
      </div>
    </div>
  </section>`
}
