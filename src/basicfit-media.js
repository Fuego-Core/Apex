...
function media(g){
 if(g.src)return `<a class="bf-media" href="${g.url}" target="_blank" rel="noopener"><img src="${g.src}" alt="Démonstration ${g.label} — ${g.source}" loading="lazy"><span>Démo officielle ${g.source}</span></a>`
 return `<a class="bf-reference" href="${g.url}" target="_blank" rel="noopener"><div><span class="bf-source">FICHE OFFICIELLE ${g.source}</span><strong>${g.label}</strong><small>Toucher pour ouvrir la démonstration complète</small></div><b>↗</b></a>`
}
function renderGuide(g){return `<section class="bf-machine"><div class="bf-section-label">COMMENT FAIRE</div>${media(g)}<div class="bf-guide"><div class="bf-title"><strong>${g.label}</strong><small>${g.muscles}</small></div><div class="bf-steps"><div><b>1</b><p><strong>Réglage</strong><span>${g.setup}</span></p></div><div><b>2</b><p><strong>Mouvement</strong><span>${g.move}</span></p></div><div class="bf-warning"><b>!</b><p><strong>À éviter</strong><span>${g.avoid}</span></p></div></div></div></section>`}
function enhance(){
 if(!location.hash.startsWith('#workout/'))return
 document.querySelectorAll('.exercise-card').forEach(card=>{
  card.querySelectorAll('.ex-visual').forEach(el=>el.remove())
  const name=card.querySelector('h2')?.textContent||'',g=findGuide(name);if(!g)return
  card.classList.add('has-official-guide')
  const cue=card.querySelector('.exercise-cues p');if(cue)cue.remove()
  let guide=card.querySelector('.bf-machine')
  if(!guide){
    const anchor=card.querySelector('.exercise-cues');if(!anchor)return
    anchor.insertAdjacentHTML('afterend',renderGuide(g));guide=card.querySelector('.bf-machine')
  }
...
let busy=false
const apply=()=>{if(busy)return;busy=true;requestAnimationFrame(()=>{try{enhance()}finally{busy=false}})}
